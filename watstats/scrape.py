...
class Scrape(object):
    ...
    def scrape(self):
        ...
        """
        watcard = requests.get('https://watcard.uwaterloo.ca/OneWeb/Financial/Transactions').text

        soup = BeautifulSoup(watcard)

        for article in soup.find('tr'):
            if "MUDIES" in article:
                print(article)
        """
